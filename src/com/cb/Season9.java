...
public class Season9 {
	...
	public static void pre() {
		...
		for (String filepath : list) {
			eccs.calEcc(filepath, filepath.replaceAll(".txt", "_ecc.txt").replaceAll("数据集", "不同数据集的权值\\\\ecc"));
		}
	}
	...
	public static void core() {
				...
				while ((buf = readBuf.readLine()) != null) {
					...
					if (flag) {
						...
						if (countMap.containsKey(buf)) {
							...
							countMap.put(buf, countMap.get(buf) + 1);
		...
		files = null;
		files = CommonUtils.getFilesInPath("H:\\金山网盘\\data\\CB\\不同数据集的权值\\ecc");
		
		for (String filepath : files) {
			List<String> eccData = CommonUtils.getInputFile(filepath);
			...
			for (Map.Entry<String, List<String>> entry : netMap.entrySet()) {
				...
				for (int i = 0; i < entry.getValue().size(); i++) {
					...
					if (countMap.containsKey(entry.getValue().get(i))) {
						value = (countMap.get(entry.getValue().get(i)) + 1) * 1.0 / (max + 1);
					} else {
						value = 1.0 / (max + 1);
					}
			...
			CommonUtils.outputFile(filepath.replaceAll("ecc", "uc"), outList);
		}
	}
